...
import type { ExtractedPage } from "./analyzer.js";
  ...
  const schemaEntries = extractedPages
    .map((page) => {
      const fieldEntries = page.fields
        .map((f) => {
          return `    ${stableStringify(f.fieldName)}: getPrimitive(${stableStringify(
            f.primitiveType
          )}).validation as z.ZodTypeAny`;
        })
        .join(",\n");
...
`;
  ...
  fs.writeFileSync(
    path.join(generatedLibDir, "pageFields.ts"),
    pageFieldsTs,
    "utf8"
  );
